...
namespace O3DWB
{
    ...
    public class Inspector : ScriptableObject, IMessageListener
    {
        ...
        {
            if (GUILayout.Button(GetContentForToggleWireframeButton(), GUILayout.Width(EditorGUILayoutEx.PreferedActionButtonWidth * 0.51f)))
            {
                Octave3DWorldBuilder.ActiveInstance.ToggleSceneObjectHighlight();
            }
        }
        ...
        private GUIContent GetContentForToggleWireframeButton()
        {
            var content = new GUIContent();
            content.text = "Toggle wireframe";
            content.tooltip = "Toggles the object wireframe which is displayed in the scene view.";
